...
```{r}
crime_ward_13 <- crime_ward[,c(3, 9, 14, 24, 29, 34, 39, 44, 49, 54, 59, 64)]
names(crime_ward_13) <- c('ward', 'life_exp', 'child_ob', 'unemp', 'crime', 'fires', 'gcse', 'absence', 'out_of_work', 'public_trans', 'greenspace', 'well_being')
```
...
```{r}
lin_full <- lm(crime ~ life_exp + child_ob + unemp + crime + fires + gcse + absence + out_of_work + public_trans + greenspace , data = crime_ward_13)
lin1 <- lm(crime ~ unemp + life_exp + absence + public_trans, data = crime_ward_13)
```

```{r}
stargazer(list(lin_full, lin1), type = 'text', digits = 2)
```
