...
Category    | Variable            | 
----------- | ------------------  | -----------------------------------------------
Health      | Life Expectancy     | Reversed index of life-expectancies for a     
-           | -                   | rolling 5-year period
-           | Childhood obesity   | Prevalence of obesity by area of child residence
-           | Incapacity benefits | Claimant rate of Incapacity Benefit or Severe
-           | claimant rate       | Disablement Allowance
Economic    | Unemployment rate   | Percent of working-age population claiming 
Security    | -                   | Jobseeker's Allowance (JSA) or National
-           | -                   | Insurance Credits
Safety      | Crime rate          | Notifiable offences per 1,000 daytime pop.
-           | Deliverate fires    | Deliberate fires per 1000 population
Education   | GCSE point scores   | Average GCSE point scores
Children    | Unauthorised pupil  | Absence in all maintained schools without 
-           | absence             | permission from authorised representative of
-           | -                   | the school
Families    | Children in out of  | % of children dependent on a parent of guardian 
-           | work households     | claiming out-of-work benefits
Access      | Public transport    | Measure of the accessibility of a point to the 
-           | Accessibility Score | public transport network
Environment | Access to public    | Measure of proportion of area that is greenspace
-           | open space          | and accessibility from homes
Happiness   | Subjective well-    | Composite Score of Happiness, Life Satisfaction,
-           | being avg score     | Worthwhileness and Anxiety
...
The correlation analysis in this report adopts the Pearson correlation coefficient, denoted r, with the aim to examine the strength and direction of the linear relationship between two continuous variables, i.e. crime rate and one of the other factors. The correlation coefficient ranges from −1 to +1, and a stronger relationship between the two variables translates into a larger absolute value of the coefficient. A positive coefficient means both variables tend to increase or decrease together whereas a negative coefficient means that the two variables move in opposite directions (Zhou et al., 2016).

The following analysis does not conclude whether the relationship between the two variables is causal of nature as this question will be addressed in the regression section.


##Test of correlation 
The following table denotes the correlation between crime rates and several indicators of well-being along with their corresponding significance.
...
The significance level refers to the risk associated with concluding a significant relationship when in fact there is none. A standard of a 95 per cent confidence interval will be used throughout this report as we believe this level to be scientifically robust and fair. Therefore, if the p-value of a correlation test is less than or equal to α, we would conclude that the correlation is statistically significant and the variable will be included in further discussion.
...
### Description
These three health-realated indexes including rolling 5-year combined life expectancies, prevalence of obesity by area of child residence, and incapacity benefit or severe disablement allowance claimant rate.
### Life Expectancy
...
An et al. (2017) revealed a  modest relationship between residential neighbourhood safety and weight-related behaviours/outcomes among children and adolescents through examining 22 studies, Half of which conducted in the USA, and one of which was conducted in a developing country. There analysis suggested that living in district with relatively high crime rate  was associated with a reduction in children's physical activity, and therefore gain in body mass index, but no change in childhood obesity risk.
...
It is noticable that except for in the year 2013, wards that have extremely high childhood obesity score tend to have lower crime rate, the phenomenon violate the moderate positive relationship between the two variables.
...
Although the maximum incapacity benefit rate decreased continuously from around 15% in 2009 to around 5% in 2013, wards with highest incapacity benefit rates have relatively low crime rates in all 5 years.  
...
```
As we can see from the scatters, the relationships are consistent from our expectation. However, correlations (-0.33 and 0.374) between education variables and crime rate are relatively weaker compared with unemployment. Both of the variables are all related to primary school education. If we can also combine data from secondary school or even university to create a complete picture of education, it may reveal a stronger relationship with crime rate. 

### Dependent children

Dependent children shows the percentage of children living in out-of-work benefit claimant households. A moderate positive correlation indicates that boroughs with higher dependent children percentage tends to have higher crime rate.
```{r}
...
```
The correlation between crime rate and figure of deliberate fires per 1000 population is very weak throughout the 5 years, correlation figure fluctuates around 0.13. Although deliberate fire expects to be a great indicator, as this directly relates to crime rate; the correlation figure shows a weak relationship. It is suggested to have further investigations of this factor
...
##Correlation Matrix
Finally, we study the correlation between any two variables we study by constructing a correlation matrix.
```{r}
cor_mat_df<-round(cor(select(crime_ward_2013, -Ward, -Borough)),4)
kable(cor_mat_df)
